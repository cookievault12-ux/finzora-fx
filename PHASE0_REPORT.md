...
Added 22 Aug 2026, during Phase 3, per the owner's explicit request to be reminded of these before any live-signal decision — do not let any of them slide by default just because paper trading is working:
...
3. **Saxo as a second broker/execution provider.** Added 22 Aug 2026. The owner obtained a Saxo demo-account API key (24-hour-lived, per Saxo's demo/SIM token policy) and updated it, but explicitly decided to shelve real Saxo integration until just before live trading — deliberately not building `SaxoProvider` now given the token is short-lived and not worth wiring up for paper-trading-only use. The owner's stated plan: get an actual (non-demo) Saxo account whose key does not expire, then revisit. Today, Saxo is config/documentation only — `SAXO_API_TOKEN`/`SAXO_ACCOUNT_ID`/`SAXO_ENVIRONMENT` are placeholders in `.env.example`, `config/providers.yaml` lists it as the documented "alternate" broker, and `src/brokers/base.py`'s docstring mentions it as a future adapter target — but no `src/providers/saxo.py` or `src/brokers/saxo.py` exists, and nothing in the running system reads that token. When this is picked back up: (a) confirm the new Saxo account's key really is non-expiring (or, if it's still a refresh-token-style OAuth setup, build an automated refresh loop rather than expecting manual key rotation), and (b) decide whether Saxo becomes the second live *price* source (item 2 above) as well as a second broker, since it's already MAS-licensed for leveraged spot FX per the Phase 0 broker comparison (section on Broker Comparison, Saxo scored 8/10 vs OANDA's 8.5/10).

**Action for whoever (human or Claude) is planning the live-trading cutover: read this section first and raise all three items explicitly before proceeding, even if not otherwise prompted.**
